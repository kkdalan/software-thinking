...
為了防止「重複交易」與「重複補償」，系統設計時可以將設計**冪等性**操作，確保無論同一操作被執行多少次，結果都保持一致。以下我們介紹如何實現**冪等性**操作。
...
另外，我們也探討在網路環境不穩定的情況下所可能導致「交易懸掛」或「空補償」的情況，以及衍生出「重複交易」與「重複補償」等問題，可以透過設計冪等性操作，來應對這些挑戰，保障系統的穩定性與可恢復性。
